...
(function (factory) {
	...
	var MDTimePicker = function (el, config) {
		...
		this.timepicker = {
			...
			timeHolder: {
				wrapper: $('<section class="mdtp__time_holder bg-primary-600"></section>'),
			...
			clockHolder: {
				wrapper: $('<section class="mdtp__clock_holder bg-white dark:bg-gray-800 text-gray-900  dark:text-white"></section>'),
				am: $('<span class="mdtp__am">AM</span>'),
				pm: $('<span class="mdtp__pm">PM</span>'),
				clock: {
					wrapper: $('<div class="mdtp__clock dark:bg-gray-700"></div>'),
				...
				buttonsHolder: {
					...
					btnOk: $(`<button class="filament-button filament-button-size-md inline-flex items-center justify-center py-1 gap-1 font-medium rounded-lg border transition-colors focus:outline-none focus:ring-offset-2 focus:ring-2 focus:ring-inset dark:focus:ring-offset-0 min-h-[2.25rem] px-4 text-sm text-white shadow focus:ring-white border-transparent bg-primary-600 hover:bg-primary-500 focus:bg-primary-700 focus:ring-offset-primary-700 filament-page-button-action ok">${
                        this.config.okLabel || "Ok"
              }</button>`),
					btnCancel: $(`<button class="my-2 filament-button filament-button-size-md inline-flex items-center justify-center py-1 gap-1 font-medium rounded-lg border transition-colors focus:outline-none focus:ring-offset-2 focus:ring-2 focus:ring-inset dark:focus:ring-offset-0 min-h-[2.25rem] px-4 text-sm text-gray-800 bg-white border-gray-300 hover:bg-gray-50 focus:ring-primary-600 focus:text-primary-600 focus:bg-primary-50 focus:border-primary-600 dark:bg-gray-800 dark:hover:bg-gray-700 dark:border-gray-600 dark:hover:border-gray-500 dark:text-gray-200 dark:focus:text-primary-400 dark:focus:border-primary-400 dark:focus:bg-gray-800 filament-page-button-action cancel">${
						this.config.cancelLabel || "Cancel"
					  }</button>`)
				}
			}
		};
	...
	$.fn.mdtimepicker.defaults = {
		...
		format: 'hh:mm:ss',			// format of the input value
...
function mdtimepicker(el,config) {
    return {
        state: config.state,
        init: function () {
            $(el).mdtimepicker(config.config).on('timechanged', function (e) {
                this.state = e.time;
            }.bind(this));
        }
    }
